...
export function validateAsanaXml(xmlString: string): string[] {
  ...
  if (xmlString.trim() === "") {
    return [
      "Input XML string cannot be just whitespace. Use <body></body> for empty content.",
    ];
  }
  ...
  if (parseErrors.length > 0) {
    return [`XML is not well-formed: ${parseErrors[0]}`];
  }
  ...
  if (!rootElement) {
    ...
    return [
      "XML document is empty or lacks a root element. Must start with <body>.",
    ];
  }
  ...
  function validateNode(node: Element, lineage: string) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (!asanaRichTextRules.tagsWithAllowedAttributes.has(tagName)) {
        addError(
          `Tag <${tagName}> does not support attributes, but found '${attrName}' ${errorMsgContext}`,
        );
      ...
      if (!allowedAttrsForTag || !allowedAttrsForTag.has(attrName)) {
        addError(
          `Unsupported attribute '${attrName}' found ${errorMsgContext}`,
        );
      }
    ...
    if (asanaRichTextRules.emptyTags.has(tagName)) {
      ...
      if (hasContent) {
        addError(
          `Tag <${tagName}> must be empty but contains content at ${currentLineage}`,
        );
      }
    }
    ...
    const structuralRule =
      asanaRichTextRules.structuralRules[
        tagName as keyof typeof asanaRichTextRules.structuralRules
      ];
    if (structuralRule?.requiredParents) {
      ...
      if (!parent || !structuralRule.requiredParents.has(parent.tagName)) {
        ...
        const expectedParents = Array.from(structuralRule.requiredParents)
          .map((p) => `<${p}>`)
          .join(" or ");
        addError(
          `Tag <${tagName}> at ${currentLineage} must be a direct child of ${expectedParents}, but its parent is ${parentTagName}.`,
        );
      }
    }

    // --- 4e. Validate Children (Elements and Text) ---
    const allowedChildElementsSet =
      asanaRichTextRules.allowedChildElementTags[
        tagName as keyof typeof asanaRichTextRules.allowedChildElementTags
      ];
    const allowDirectText =
      !asanaRichTextRules.disallowedDirectTextChildren.has(tagName);
    ...
    for (let ci = 0; ci < node.childNodes.length; ci++) {
      ...
      if (childNode.nodeType === 1 /* Node.ELEMENT_NODE */) {
        ...
        if (
          !parentAllowsAnyChildElement &&
          !allowedChildElementsSet.has(childTagName)
        ) {
          addError(
            `Tag <${childTagName}> is not allowed as a direct child of <${tagName}> at ${currentLineage}`,
          );
      ...
      // Validate Text Nodes
        ...
        if (!allowDirectText) {
          ...
          addError(
            `Text content ("${childNode.nodeValue?.trim().substring(0, 20)}...") found directly inside <${tagName}> at ${currentLineage}, which is not allowed.`,
          );
        } else if (
          allowedChildElementsSet?.size === 0 &&
          tagName !== "code" &&
          tagName !== "pre" &&
          tagName !== "h1" &&
          tagName !== "h2"
